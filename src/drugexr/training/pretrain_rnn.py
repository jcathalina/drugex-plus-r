...
from src.drugexr.config.constants import PROC_DATA_PATH, MODEL_PATH
...
def pretrain_rnn(is_lstm: bool = True):
    voc = Vocabulary(vocabulary_path=pathlib.Path(PROC_DATA_PATH / "vocabulary.txt"))

    out_dir = MODEL_PATH / 'output/rnn'
    if not pathlib.Path.exists(out_dir):
        logging.info(
            f"Creating directories to store pretraining output @ '{out_dir}'"
        )
